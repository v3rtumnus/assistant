...
public class AnonymizationService {
    ...
    private List<PatternDefinition> initializePatterns() {
        ...
        // ==================== SMART HOME - DEVICES ====================
        // German device names - Window coverings
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Jalousie[n]?|Rollo[s]?|Rolllade[n]?|Rollladen|Rollläden|Markise[n]?|Vorhang|Vorhänge|Vorhaenge|Gardine[n]?|Plissee[s]?|Lamellenvorhang|Lamellenvorhänge|Raffstore[s]?|Außenjalousie[n]?|Aussenjalousie[n]?|Innenjalousie[n]?|Verdunkelung|Verdunkelungsrollo[s]?|Sonnenschutz|Sichtschutz|Beschattung)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Window coverings
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:blind[s]?|shutter[s]?|curtain[s]?|drape[s]?|shade[s]?|awning[s]?|roller blind[s]?|venetian blind[s]?|vertical blind[s]?|roman shade[s]?|blackout blind[s]?|window covering[s]?)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // German device names - Lighting
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Licht(?:er)?|Lampe[n]?|Beleuchtung|Deckenlampe[n]?|Deckenleuchte[n]?|Stehlampe[n]?|Wandlampe[n]?|Tischlampe[n]?|Nachttischlampe[n]?|Spotlicht(?:er)?|Spot[s]?|LED[s]?|LED-Streifen|Lichtstreifen|Lichtband|Außenbeleuchtung|Aussenbeleuchtung|Gartenbeleuchtung|Wegbeleuchtung|Eingangsbeleuchtung|Dimmbare?\\s*(?:Licht(?:er)?|Lampe[n]?)|Hauptlicht|Ambientelicht|Leselicht|Arbeitslicht)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Lighting
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:light[s]?|lamp[s]?|lighting|ceiling light[s]?|ceiling lamp[s]?|floor lamp[s]?|wall lamp[s]?|table lamp[s]?|bedside lamp[s]?|spotlight[s]?|LED[s]?|LED strip[s]?|light strip[s]?|outdoor light[s]?|garden light[s]?|pathway light[s]?|porch light[s]?|dimmable light[s]?|main light|ambient light|reading light|task light)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // German device names - Climate
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Heizung|Thermostat[e]?|Klimaanlage[n]?|Klimagerät[e]?|Lüftung|Lueftung|Ventilator(?:en)?|Lüfter|Luefter|Heizkörper|Heizkoerper|Fußbodenheizung|Fussbodenheizung|Radiatoren?|Wärmepumpe[n]?|Waermepumpe[n]?|Luftentfeuchter|Luftbefeuchter|Raumtemperatur|Luftreiniger|Dunstabzug|Dunstabzugshaube|Abzugshaube)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Climate
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:heating|thermostat[s]?|air conditioner[s]?|AC|HVAC|ventilation|fan[s]?|radiator[s]?|floor heating|underfloor heating|heat pump[s]?|dehumidifier[s]?|humidifier[s]?|air purifier[s]?|range hood|exhaust fan[s]?)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // German device names - Security & Doors
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Tür(?:en)?|Tuere?n?|Haustür|Haustuere?|Eingangstür|Eingangstuer|Garagentor|Gartentor|Fenster|Dachfenster|Türschloss|Tuerschloss|Smartlock|Alarmanlage[n]?|Sicherheitssystem[e]?|Überwachungskamera[s]?|Kamera[s]?|Bewegungsmelder|Rauchmelder|CO-Melder|Kohlenmonoxidmelder|Wassersensor(?:en)?|Türklingel|Tuerklingel|Video-?Türklingel|Gegensprechanlage[n]?|Türöffner|Tueroeffner)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Security & Doors
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:door[s]?|front door|back door|garage door|garden gate|window[s]?|skylight[s]?|door lock[s]?|smart lock[s]?|alarm system[s]?|security system[s]?|security camera[s]?|camera[s]?|motion sensor[s]?|smoke detector[s]?|smoke alarm[s]?|CO detector[s]?|carbon monoxide detector[s]?|water sensor[s]?|leak sensor[s]?|doorbell[s]?|video doorbell[s]?|intercom[s]?|door opener[s]?)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // German device names - Appliances
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Fernseher|TV|Fernsehgerät|Beamer|Projektor|Lautsprecher|Soundbar|Stereoanlage|Musikanlage|Kaffeemaschine[n]?|Kaffeevollautomat(?:en)?|Waschmaschine[n]?|Trockner|Wäschetrockner|Geschirrspüler|Spülmaschine[n]?|Kühlschrank|Kuehlschrank|Gefrierschrank|Backofen|Herd|Kochfeld|Mikrowelle[n]?|Staubsauger(?:roboter)?|Mähroboter|Maehroboter|Rasenmäher|Rasensprenger|Bewässerung|Bewaesserung|Poolpumpe|Whirlpool|Sauna|Saunasteuerung)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Appliances
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:TV|television[s]?|projector[s]?|speaker[s]?|soundbar[s]?|stereo|music system[s]?|coffee maker[s]?|coffee machine[s]?|washing machine[s]?|washer[s]?|dryer[s]?|dishwasher[s]?|refrigerator[s]?|fridge[s]?|freezer[s]?|oven[s]?|stove[s]?|cooktop[s]?|microwave[s]?|vacuum cleaner[s]?|robot vacuum[s]?|lawn mower[s]?|robot mower[s]?|sprinkler[s]?|irrigation|pool pump[s]?|hot tub[s]?|jacuzzi)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // German device names - Smart Home General
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:Steckdose[n]?|Smartplug[s]?|Smart-?Steckdose[n]?|Schalter|Smart-?Schalter|Dimmer|Bewegungssensor(?:en)?|Temperatursensor(?:en)?|Luftfeuchtigkeitssensor(?:en)?|Helligkeitssensor(?:en)?|Fenstersensor(?:en)?|Türsensor(?:en)?|Tuersensor(?:en)?|Kontaktsensor(?:en)?|Energiemessgerät[e]?|Stromzähler|Gaszähler|Wasserzähler|Wallbox|Ladestation(?:en)?)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));

        // English device names - Smart Home General
        patterns.add(new PatternDefinition(
                EntityType.HOME_DEVICE,
                Pattern.compile("\\b(?:outlet[s]?|smart plug[s]?|smart outlet[s]?|switch(?:es)?|smart switch(?:es)?|dimmer[s]?|motion sensor[s]?|temperature sensor[s]?|humidity sensor[s]?|light sensor[s]?|window sensor[s]?|door sensor[s]?|contact sensor[s]?|energy monitor[s]?|power meter[s]?|gas meter[s]?|water meter[s]?|EV charger[s]?|charging station[s]?|wallbox)\\b", Pattern.CASE_INSENSITIVE),
                0.90,
                null
        ));
